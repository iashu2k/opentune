...
def serialize_example(ex: dict) -> str:
    ...
    parts: list[str] = []
    pre = " ".join(t.strip() for t in ex.get("pre_text", []) if t.strip())
    if pre:
        parts.append(pre)
    table = ex.get("table", [])
    if table:
        rows = [" | ".join(str(c).strip() for c in row) for row in table]
        parts.append("\n".join(rows))
    post = " ".join(t.strip() for t in ex.get("post_text", []) if t.strip())
    if post:
        parts.append(post)
    return "\n".join(parts)
...
def build_split(split: str) -> tuple[pd.DataFrame, Counter]:
    ...
    for i, ex in enumerate(raw):
        ...
        if gold is None:
            reason = "empty_gold" if not (qa.get("answer") or qa.get("exe_ans")) else "non_numeric_gold"
            dropped[reason] += 1
            continue
